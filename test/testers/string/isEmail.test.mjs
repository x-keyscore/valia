...
describe("Testers string 'isEmail' function", () => {
	...
	it("'allowAddressLiteral' parameter (ipV6)", () => {
		assert.strictEqual(isEmail("foo@[0000:0000:0000:0000:0000:0000:0000:00001]", { allowAddressLiteral: true }), false);
		assert.strictEqual(isEmail("foo@[::ffff::]", { allowAddressLiteral: true }), false);
		assert.strictEqual(isEmail("foo@[::ffff:192.168.1.256]", { allowAddressLiteral: true }), false);

		assert.strictEqual(isEmail("foo@[0000:0000:0000:0000:0000:0000:0000:0000]", { allowAddressLiteral: true }), true);
		assert.strictEqual(isEmail("foo@[::ffff]", { allowAddressLiteral: true }), true);
		assert.strictEqual(isEmail("foo@[::ffff:192.168.1.255]", { allowAddressLiteral: true }), true);
	});
});