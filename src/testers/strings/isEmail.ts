 ...
 * `ALPHA` and `DIGIT`
 */
function isAlphanumeric(codePoint: number) {
	// ALPHA
 ...
 * `atext`
 */
function isAtextChar(codePoint: number) {
	// ALPHA
	if ((codePoint | 32) >= 97 && (codePoint | 32) <= 122) return (true);
	// DIGIT
	if (codePoint >= 48 && codePoint <= 57) return (true);
 ...
 * `qtextSMTP`
 */
function isQtextSMTPChar(codePoint: number) {
	// %d32-33
	if (codePoint >= 32 && codePoint <= 33) return (true);
	// %%d35-91
	if (codePoint >= 35 && codePoint <= 91) return (true);
	// %d93-126
	if (codePoint >= 93 && codePoint <= 126) return (true);
 ...
 */
function isQpairSMTPChar(codePoint: number) {
	// %d32-126
 ...
 * ` dcontent`
 */
function isDcontentChar(codePoint: number) {
	// %d33-90
	if (codePoint >= 33 && codePoint <= 90) return (true);
	// %d94-126
	if (codePoint >= 94 && codePoint <= 126) return (true);
 ...
 */
function skipQuotedString(utf16UnitArray: Uint16Array, startIndex: number) {
	...
	while (i < arrayLength) {
		...
		if (code === 34) {// "\""
			...
			if (++i > arrayLength || !isQpairSMTPChar(utf16UnitArray[i])) return (-1);
 ...
 * `1*atext *("."  1*atext)`
 */
function skipDotString(utf16UnitArray: Uint16Array, startIndex: number) {
	const arrayLength = utf16UnitArray.length;
	let i = startIndex;
	let startAtom = 1;
	...
	while (i < arrayLength) {
		...
		if (code === 46) {// "."
			if (startAtom) return (-1);
			startAtom = 1;
		} else if (isAtextChar(code)) {
			if (startAtom) startAtom = 0;
		} else {
			break;
		}
 ...
 */
function isValidLocalPart(utf16UnitArray: Uint16Array, allowQuotedString: boolean) {
	let i = 0;
	...
	if (i !== utf16UnitArray.length) return (false);
 ...
 */
function isGeneralAddressLiteral(utf16UnitArray: Uint16Array) {
	...
	if (i < 1 || i === utf16UnitArray.length - 1) return (false);
	colonIndex = i--;
	...
	while (i >= 0) {
		...
		if (code === 45) {// "-"
			if (colonIndex - 1 === i) return (false);
 ...
 */
function isAddressLiteral(utf16UnitArray: Uint16Array) {
	const arrayLength = utf16UnitArray.length;

	// CHECK START BRACKET
	if (utf16UnitArray[0] !== 91) return (false);// "["
 ...
 */
function isDomain(utf16UnitArray: Uint16Array) {
	const arrayLength = utf16UnitArray.length;
	let prevIsDot = 1;
	let prevIsHyp = 0;
	let i = 0;
 ...
 */
function isValidDomainPart(utf16UnitArray: Uint16Array, allowAddressLiteral: boolean) {
 ...
 * `Mailbox`
 */
function extractLocalAndDomain(utf16UnitArray: Uint16Array) {
	const arrayLength = utf16UnitArray.length;
	...
	// CHECK SYMBOL CHAR
	if (utf16UnitArray[i] !== 64) return (null);// "@"
 ...
 * `Mailbox`
 */
export function isEmail(input: string | Uint16Array, params?: IsEmailParams) {
	...
	if (!isValidLocalPart(parts.local, params?.allowQuotedString || false)) return (false);

	if (!isValidDomainPart(parts.domain, params?.allowAddressLiteral || false)) return (false);
