...
type NumberGuard<T extends VariantCriteria> = T extends NumberVariantCriteria ? T['enum'] extends number[] ? T['enum'][number] : T['enum'] extends Record<string, number> ? {
    [K in keyof T['enum']]: T['enum'][K];
}[keyof T['enum']] : number : never;
